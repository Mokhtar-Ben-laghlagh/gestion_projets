...
public class AffectationServiceImpl implements AffectationService {
    ...
    public AffectationResponse update(Long phaseId, Long empId, AffectationRequest r) {
        ...
        if (r.getDateDebut().isBefore(phase.getDateDebut()) || r.getDateFin().isAfter(phase.getDateFin()))
            throw new InvalidDateException("Dates d'affectation hors de la plage de la phase.");
        a.setDateDebut(r.getDateDebut());
        a.setDateFin(r.getDateFin());
        a.setDescription(r.getDescription());
        a.setRole(r.getRole());
        a.setStatut(r.getStatut());
        return toResp(repo.save(a));
    }

    @Transactional(readOnly = true)
    public AffectationResponse getOne(Long phaseId, Long empId) {
        return toResp(repo.findById(new AffectationId(empId, phaseId)).orElseThrow(() -> new ResourceNotFoundException("Affectation introuvable")));
    }

    @Transactional(readOnly = true)
    public List<AffectationResponse> getByPhase(Long pId) {
        return repo.findByPhaseId(pId).stream().map(this::toResp).collect(Collectors.toList());
    }

    @Transactional(readOnly = true)
    public List<AffectationResponse> getByEmploye(Long eId) {
        return repo.findByEmployeId(eId).stream().map(this::toResp).collect(Collectors.toList());
    }

    public void delete(Long phaseId, Long empId) {
        AffectationId aid = new AffectationId(empId, phaseId);
        if (!repo.existsById(aid)) throw new ResourceNotFoundException("Affectation introuvable");
        repo.deleteById(aid);
    }

    private AffectationResponse toResp(Affectation a) {
        AffectationResponse r = new AffectationResponse();
        r.setEmployeId(a.getEmploye().getId());
        r.setPhaseId(a.getPhase().getId());
        r.setEmployeNom(a.getEmploye().getNom());
        r.setEmployePrenom(a.getEmploye().getPrenom());
        r.setEmployeMatricule(a.getEmploye().getMatricule());
