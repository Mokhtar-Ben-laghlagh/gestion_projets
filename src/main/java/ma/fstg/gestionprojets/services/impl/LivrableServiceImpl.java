...
public class LivrableServiceImpl implements LivrableService {
    ...
    public LivrableResponse update(Long id, LivrableRequest r) {
        Livrable l = find(id);
        l.setCode(r.getCode());
        l.setLibelle(r.getLibelle());
        l.setDescription(r.getDescription());
        l.setChemin(r.getChemin());
        l.setTypeFichier(r.getTypeFichier());
        l.setDateRemise(r.getDateRemise());
        if (r.getValide() != null) l.setValide(r.getValide());
        return toResp(repo.save(l));
    }
    ...
    @Transactional(readOnly = true)
    public List<LivrableResponse> getByPhase(Long pId) {
    ...
    private Livrable find(Long id) {
        return repo.findById(id).orElseThrow(() -> new ResourceNotFoundException("Livrable", id));
    }
    ...
    private LivrableResponse toResp(Livrable l) {
        ...
        r.setChemin(l.getChemin());
        r.setTypeFichier(l.getTypeFichier());
        r.setDateRemise(l.getDateRemise());
        r.setValide(l.getValide());
        if (l.getPhase() != null) {
            r.setPhaseId(l.getPhase().getId());
            r.setPhaseLibelle(l.getPhase().getLibelle());
        }
        return r;
    }
}
