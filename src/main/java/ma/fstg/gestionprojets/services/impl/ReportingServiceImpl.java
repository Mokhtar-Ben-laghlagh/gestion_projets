...
public class ReportingServiceImpl implements ReportingService {
    ...
    public List<PhaseResponse> getPhasesTermineesNonFacturees() {
        return phaseRepo.findByEtatRealisationTrueAndEtatFacturationFalse().stream().map(phaseService::toResponse).collect(Collectors.toList());
    }

    public List<PhaseResponse> getPhasesFactureesNonPayees() {
        return phaseRepo.findByEtatFacturationTrueAndEtatPaiementFalse().stream().map(phaseService::toResponse).collect(Collectors.toList());
    }

    public List<PhaseResponse> getPhasesPayees() {
        return phaseRepo.findByEtatPaiementTrue().stream().map(phaseService::toResponse).collect(Collectors.toList());
    }

    public List<ProjetResponse> getProjetsEnCours() {
        return projetRepo.findEnCours(LocalDate.now()).stream().map(projetService::toResp).collect(Collectors.toList());
    }

    public List<ProjetResponse> getProjetsClotures() {
        return projetRepo.findClotures(LocalDate.now()).stream().map(projetService::toResp).collect(Collectors.toList());
    }
}
