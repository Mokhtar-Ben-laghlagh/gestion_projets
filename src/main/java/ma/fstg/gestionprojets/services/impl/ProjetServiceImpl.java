...
public class ProjetServiceImpl implements ProjetService {
    ...
    public ProjetResponse create(ProjetRequest r) {
        if (projetRepo.existsByCode(r.getCode()))
            throw new DuplicateResourceException("Code projet '" + r.getCode() + "' déjà existant.");
    ...
    public ProjetResponse update(Long id, ProjetRequest r) {
        Projet p = find(id);
        if (!p.getCode().equals(r.getCode()) && projetRepo.existsByCode(r.getCode()))
        ...
        p.setDateDebut(r.getDateDebut());
        p.setDateFin(r.getDateFin());
        p.setMontant(r.getMontant());
        p.setOrganisme(org);
    ...
    public List<ProjetResponse> getAll() {
        return projetRepo.findAll().stream().map(this::toResp).collect(Collectors.toList());
    }
    ...
    @Transactional(readOnly = true)
    public List<ProjetResponse> getEnCours() {
        return projetRepo.findEnCours(LocalDate.now()).stream().map(this::toResp).collect(Collectors.toList());
    }

    @Transactional(readOnly = true)
    public List<ProjetResponse> getClotures() {
        return projetRepo.findClotures(LocalDate.now()).stream().map(this::toResp).collect(Collectors.toList());
    }

    public void delete(Long id) {
        Projet p = find(id);
        if (p.getPhases() != null && !p.getPhases().isEmpty())
            throw new BusinessException("Impossible de supprimer un projet avec des phases.");
        projetRepo.deleteById(id);
    }

    private Projet find(Long id) {
        return projetRepo.findById(id).orElseThrow(() -> new ResourceNotFoundException("Projet", id));
    }
