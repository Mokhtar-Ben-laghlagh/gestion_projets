...
public class EmployeServiceImpl implements EmployeService {
    ...
    public EmployeResponse create(EmployeRequest r) {
        if (employeRepo.existsByMatricule(r.getMatricule()))
        ...
        if (r.getEmail() != null && employeRepo.existsByEmail(r.getEmail()))
    ...
    public EmployeResponse update(Long id, EmployeRequest r) {
        Employe e = find(id);
        if (!e.getMatricule().equals(r.getMatricule()) && employeRepo.existsByMatricule(r.getMatricule()))
            throw new DuplicateResourceException("Matricule déjà utilisé.");
        if (!e.getLogin().equals(r.getLogin()) && employeRepo.existsByLogin(r.getLogin()))
            throw new DuplicateResourceException("Login déjà utilisé.");
        Profil profil = r.getProfilId() != null ? profilRepo.findById(r.getProfilId()).orElseThrow(() -> new ResourceNotFoundException("Profil", r.getProfilId())) : null;
        e.setMatricule(r.getMatricule());
        e.setNom(r.getNom());
        e.setPrenom(r.getPrenom());
        e.setTelephone(r.getTelephone());
        e.setEmail(r.getEmail());
        e.setLogin(r.getLogin());
        e.setAdresse(r.getAdresse());
        e.setProfil(profil);
    ...
    public List<EmployeResponse> getAll() {
        return employeRepo.findAll().stream().map(this::toResp).collect(Collectors.toList());
    }

    @Transactional(readOnly = true)
    public List<EmployeResponse> search(String nom) {
        return employeRepo.findByNomContainingIgnoreCaseOrPrenomContainingIgnoreCase(nom, nom).stream().map(this::toResp).collect(Collectors.toList());
    }

    @Transactional(readOnly = true)
    public List<EmployeResponse> getDisponibles(LocalDate d, LocalDate f) {
        return employeRepo.findDisponibles(d, f).stream().map(this::toResp).collect(Collectors.toList());
    }
    ...
    private Employe find(Long id) {
        return employeRepo.findById(id).orElseThrow(() -> new ResourceNotFoundException("Employé", id));
    }
    ...
    public EmployeResponse toResp(Employe e) {
        ...
        r.setActif(e.getActif());
        if (e.getProfil() != null) {
            ProfilResponse p = new ProfilResponse();
            p.setId(e.getProfil().getId());
            p.setCode(e.getProfil().getCode());
            p.setLibelle(e.getProfil().getLibelle());
            r.setProfil(p);
        }
        return r;
    }
