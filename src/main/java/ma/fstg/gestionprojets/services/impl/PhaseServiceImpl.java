...
public class PhaseServiceImpl implements PhaseService {
    ...
    public PhaseResponse update(Long id, PhaseRequest r) {
        ...
        phase.setCode(r.getCode());
        phase.setLibelle(r.getLibelle());
        phase.setDescription(r.getDescription());
        phase.setDateDebut(r.getDateDebut());
        phase.setDateFin(r.getDateFin());
        phase.setMontant(r.getMontant());
        phase.setPourcentage(r.getPourcentage());
        return toResponse(phaseRepo.save(phase));
    }

    @Transactional(readOnly = true)
    public PhaseResponse getById(Long id) {
        return toResponse(findById(id));
    }

    @Transactional(readOnly = true)
    public List<PhaseResponse> getByProjet(Long pId) {
        return phaseRepo.findByProjetId(pId).stream().map(this::toResponse).collect(Collectors.toList());
    }

    public PhaseResponse updateRealisation(Long id, boolean etat) {
        Phase p = findById(id);
        p.setEtatRealisation(etat);
        return toResponse(phaseRepo.save(p));
    }

    public PhaseResponse updateFacturation(Long id, boolean etat) {
        Phase p = findById(id);
        if (etat && !Boolean.TRUE.equals(p.getEtatRealisation()))
            throw new BusinessException("La phase doit être réalisée avant d'être facturée.");
        p.setEtatFacturation(etat);
        return toResponse(phaseRepo.save(p));
    }

    public PhaseResponse updatePaiement(Long id, boolean etat) {
        Phase p = findById(id);
        if (etat && !Boolean.TRUE.equals(p.getEtatFacturation()))
    ...
    public Phase findEntityById(Long id) {
        return findById(id);
    }

    private void validateDates(PhaseRequest r, Projet projet) {
        if (r.getDateDebut().isAfter(r.getDateFin()))
            throw new InvalidDateException("Date début doit être avant date fin.");
    ...
    private void validateMontant(Long projetId, Double nouveau, Long excludeId, Double montantProjet) {
        ...
        if (excludeId != null) {
            Phase ex = phaseRepo.findById(excludeId).orElse(null);
            if (ex != null) somme -= ex.getMontant();
        }
        if (somme + nouveau > montantProjet)
            throw new MontantDepasseException("La somme des montants des phases dépasse le montant du projet (" + montantProjet + ").");
    }

    private Phase findById(Long id) {
        return phaseRepo.findById(id).orElseThrow(() -> new ResourceNotFoundException("Phase", id));
    }
    ...
    public PhaseResponse toResponse(Phase p) {
        ...
        r.setPourcentage(p.getPourcentage());
        r.setEtatRealisation(p.getEtatRealisation());
        r.setEtatFacturation(p.getEtatFacturation());
        r.setEtatPaiement(p.getEtatPaiement());
        if (p.getProjet() != null) {
            r.setProjetId(p.getProjet().getId());
            r.setProjetNom(p.getProjet().getNom());
        }
