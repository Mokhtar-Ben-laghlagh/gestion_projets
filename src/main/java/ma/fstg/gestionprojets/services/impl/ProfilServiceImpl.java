...
public class ProfilServiceImpl implements ProfilService {
    ...
    public ProfilResponse create(ProfilRequest r) {
        if (repo.existsByCode(r.getCode()))
            throw new DuplicateResourceException("Code profil '" + r.getCode() + "' déjà existant.");
    ...
    public ProfilResponse update(Long id, ProfilRequest r) {
        Profil p = findById(id);
        p.setCode(r.getCode());
        p.setLibelle(r.getLibelle());
    ...
    private Profil findById(Long id) {
        return repo.findById(id).orElseThrow(() -> new ResourceNotFoundException("Profil", id));
    }

    public ProfilResponse toResp(Profil p) {
