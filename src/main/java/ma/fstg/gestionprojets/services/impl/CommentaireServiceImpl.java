...
public class CommentaireServiceImpl implements CommentaireService {
    ...
    public List<CommentaireResponse> getByProjet(Long pId) {
        return repo.findByProjetId(pId).stream().map(this::toResp).collect(Collectors.toList());
    }

    @Transactional(readOnly = true)
    public List<CommentaireResponse> getByPhase(Long pId) {
        return repo.findByPhaseId(pId).stream().map(this::toResp).collect(Collectors.toList());
    }

    public void delete(Long id) {
        repo.findById(id).orElseThrow(() -> new ResourceNotFoundException("Commentaire", id));
        repo.deleteById(id);
    }

    private CommentaireResponse toResp(Commentaire c) {
        CommentaireResponse r = new CommentaireResponse();
        r.setId(c.getId());
        r.setContenu(c.getContenu());
        r.setDateCreation(c.getDateCreation());
        if (c.getAuteur() != null) {
            r.setAuteurNom(c.getAuteur().getNom());
            r.setAuteurPrenom(c.getAuteur().getPrenom());
        }
        if (c.getProjet() != null) r.setProjetId(c.getProjet().getId());
        if (c.getPhase() != null) r.setPhaseId(c.getPhase().getId());
        return r;
    }
}
