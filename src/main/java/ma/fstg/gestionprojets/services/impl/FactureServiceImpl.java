...
public class FactureServiceImpl implements FactureService {
    ...
    public FactureResponse create(Long phaseId, FactureRequest r) {
        ...
        if (!Boolean.TRUE.equals(phase.getEtatRealisation()))
            throw new BusinessException("La phase doit être réalisée avant facturation.");
        ...
        if (repo.existsByCode(r.getCode()))
            throw new DuplicateResourceException("Code facture '" + r.getCode() + "' déjà existant.");
    ...
    public FactureResponse update(Long id, FactureRequest r) {
        Facture f = find(id);
        f.setCode(r.getCode());
        f.setDateFacture(r.getDateFacture());
        if (r.getMontant() != null) f.setMontant(r.getMontant());
        if (r.getReference() != null) f.setReference(r.getReference());
        return toResp(repo.save(f));
    }
    ...
    private Facture find(Long id) {
        return repo.findById(id).orElseThrow(() -> new ResourceNotFoundException("Facture", id));
    }

    private FactureResponse toResp(Facture f) {
        FactureResponse r = new FactureResponse();
        r.setId(f.getId());
        r.setCode(f.getCode());
        r.setDateFacture(f.getDateFacture());
        r.setDatePaiement(f.getDatePaiement());
        r.setMontant(f.getMontant());
        r.setReference(f.getReference());
        r.setStatut(f.getStatut() != null ? f.getStatut().name() : null);
        if (f.getPhase() != null) {
            r.setPhaseId(f.getPhase().getId());
            r.setPhaseLibelle(f.getPhase().getLibelle());
            if (f.getPhase().getProjet() != null) {
                r.setProjetNom(f.getPhase().getProjet().getNom());
                if (f.getPhase().getProjet().getOrganisme() != null)
                    r.setOrganismeNom(f.getPhase().getProjet().getOrganisme().getNom());
            }
        }
        return r;
    }
